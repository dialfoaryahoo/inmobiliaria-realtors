...
public class Facturas extends javax.swing.JDialog {
   ...
   private DefaultTableModel modeloDeMiJTable; 
    ...
    private void llenarcombocodigo(){
        ...
        private void serial(){
        conn.establecer_conexion();
        String consultaserial = "select max(serial)+1 from facturas where tipo_factura = 'FACTURA'";
...
botones(false, false, false,true);
    ...
    private void jButton_bucarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_bucarActionPerformed
        ...
        String consulta ="select serial from facturas where serial = "+recibo+" and tipo_factura = 'FACTURA'";
    ...
    botones(false, false, true, false);    // TODO add your handling code here:
            ...
            //seriaimp=7;
            contimp++;
            String path="D:/Proyecto/Inmobiliaria_corregida/src/Reportes/facturacarta.jasper";
            ...
            try {
                if(contimp!=0){
                    ...
                    parametro.put("TIPO_FACTURA", "FACTURA");
...
jv.setTitle("FORMATO DE ACEPTACION");                    
    ...
    private void jTextField_codigoActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jTextField_codigoActionPerformed
        if(destino==0){
            Conexion.JOptionShowMessage("+1", "", "Seleccione a quien desea hacer la Factura");
        }
        if(destino==1){
            conn.establecer_conexion();
    ...
    private void jButton_confirmar1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_confirmar1ActionPerformed
        ...
        if(!(jRadioButton_cheque.isSelected()||jRadioButton_efectivo.isSelected())){
            ...
            String insertar="INSERT INTO facturas(tipo_factura, serial,tipo_usuario, cod_usuario, tipo_pago, ncheque, interno_externo, total, estado, usuario, fecha) " +
                "VALUES ('FACTURA', "+serial+", '"+jComboBox_destino.getSelectedItem()+"', "+cod_usuario+", '"+forma_pago+"', '"+jTextField_ncheque.getText().toUpperCase()+"', 'INTERNO', "+valor_total+", 1,  '"+acc.getUsuario()+"', now())";
            seriaimp = serial;
