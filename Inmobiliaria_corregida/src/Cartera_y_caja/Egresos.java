...
//        jButton_informes.setSelected(d);
    ...
    private void llenarcombocodigo(){
        ...
        String sql ="select nombres from conceptos2 where cod_referencia='EGRESO';";
        ...
        private void serial(){
        conn.establecer_conexion();
        String consultaserial = "select max(serial)+1 from facturas where tipo_factura = 'EGRESO'";
        ...
        public void reportes(String path){
            conn.establecer_conexion();
            Numero_a_Letra NumLetra = new Numero_a_Letra();
            String total_letras2="";
            //TEMPORAL PARA SACAR LE VALOR EN LETRAS
            String consulta2="select total from facturas where tipo_factura = 'EGRESO' ORDER BY cod_factura desc limit 1;";
...
//            String path=ruta+"/src/Reportes/egreso.jasper";            
            JasperReport jr = null;
            try {
                if(contimp!=0){
                    JDialog viewer = new JDialog(new javax.swing.JFrame(),"Recibo De Egreso", true); 
    ...
    private void initComponents() {
        ...
        jButton_nuevo.setBounds(440, 20, 150, 50);
        ...
        jButton_bucar.setBounds(590, 20, 150, 50);
        ...
        jButton_imprimir.setBounds(740, 20, 150, 50);
    ...
    private void jButton_bucarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_bucarActionPerformed
        ...
        seriaimp= Integer.parseInt(recibo);
        jButton_imprimirActionPerformed(evt);
        
        //HACER PRUEBAS MAS PROFUNDAS.
    ...
    String consulta = "select tipo_usuario from facturas where tipo_factura = 'EGRESO' ORDER BY cod_factura desc limit 1";
    ResultSet n = conn.consulta(consulta);
        try {
            while (n.next()) {                
                tipo_usuario=n.getString(1);
            }
        } catch (Exception e) {
        }
            String ruta=System.getProperty("user.dir");
            ...
            if(tipo_usuario.equals("INQUILINO")){
                path=ruta+"/src/Reportes/Egreso_01.jasper";
            }
            else if(tipo_usuario.equals("PROPIETARIO")){
                path=ruta+"/src/Reportes/Egreso_02.jasper";
            }
            else if(tipo_usuario.equals("Otros Usuarios")){
                path=ruta+"/src/Reportes/Egreso_03.jasper";            
            }
    ...
    private void jButton_confirmarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_confirmarActionPerformed
        ...
        conn.establecer_conexion();
        String Consulta = "select cod_concepto from conceptos2 where cod_referencia='EGRESO' and nombres = '"+jComboBox_codigo.getSelectedItem()+"';";
    ...
    private void jButton_confirmar1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton_confirmar1ActionPerformed
        if(!(jRadioButton_cheque.isSelected()||jRadioButton_efectivo.isSelected())){
            ...
            String insertar="INSERT INTO facturas(tipo_factura, serial,tipo_usuario, cod_usuario, tipo_pago, ncheque, interno_externo, total, estado, usuario, fecha) " +
                "VALUES ('EGRESO', "+serial+", '"+jComboBox_destino.getSelectedItem()+"', "+cod_usuario+", '"+forma_pago+"', '"+jTextField_ncheque.getText().toUpperCase()+"', 'INTERNO', "+valor_total+", 1,  '"+acc.getUsuario()+"', now())";
            seriaimp = serial;
            ...
            if(cod_factura!=0){            
                ...
                //TRAER EL VALOR, PARA CONVERTIRLO EN LETRAS
                String consulta2="select total from facturas where tipo_factura = 'EGRESO' ORDER BY cod_factura desc limit 1;";
                ResultSet rs2 = conn.consulta(consulta2);
                Numero_a_Letra NumLetra = new Numero_a_Letra();
                    try {
                        while (rs2.next()) {
                            total_letras= (rs2.getString(1));
                            total_letras=NumLetra.Convertir(total_letras, true);
                        }
...
//        limpiarTabla(jTable1);
    ...
    String forma_pago="", total_letras="";
